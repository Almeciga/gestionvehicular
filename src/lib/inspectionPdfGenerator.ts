...
async function loadImageWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      const result = await convertImageToBase64(src, controller.signal);
      clearTimeout(timer);
...
// ─── Resolve image src (use base64 if available, or empty string) ─────────────
...
function buildItemRows(items: ItemConEstadoPDF[], imageMap: Map<string, string>): string {
  return items.map((item) => `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;color:#374151;font-weight:600">${escapeHtml(item.nombre)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6">
        <span style="display:inline-block;padding:2px 10px;border-radius:999px;font-size:11px;font-weight:700;background:${estadoBg(item.estado)};color:${estadoColor(item.estado)}">${escapeHtml(estadoLabel(item.estado))}</span>
      </td>
      <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280">${escapeHtml(item.notas || '—')}</td>
    ...
    return src
        ? `<img src="${src}" alt="${escapeHtml(item.nombre)}" style="width:140px;height:105px;object-fit:cover;border-radius:6px;border:1px solid #e5e7eb;display:block" />`
        : '<div style="width:140px;height:105px;background:#f3f4f6;border-radius:6px;border:1px solid #e5e7eb;display:flex;align-items:center;justify-content:center;font-size:10px;color:#9ca3af">Img no disponible</div>';
  }).join('')}
        </div>
      </td>
    </tr>` : ''}
...
function buildPhotoGallerySection(
  ...
  let html = `<div style="margin-bottom:20px">
    <p style="font-size:12px;font-weight:800;color:#1B4F72;margin-bottom:10px;padding:6px 10px;background:#f0f4f8;border-left:3px solid #1B4F72;border-radius:0 4px 4px 0">${escapeHtml(label)}</p>`;

  withPhotos.forEach((item) => {
    html += `<div style="margin-bottom:12px;page-break-inside:avoid">
      <p style="font-size:11px;font-weight:700;color:#374151;margin-bottom:6px">${escapeHtml(item.nombre)}</p>
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        ${item.fotos.map((f) => {
      const src = resolveImg(f, imageMap);
      return src
          ? `<img src="${src}" alt="${escapeHtml(item.nombre)}" style="width:180px;height:135px;object-fit:cover;border-radius:8px;border:1px solid #e5e7eb;display:block" />`
          : '<div style="width:180px;height:135px;background:#f3f4f6;border-radius:8px;border:1px solid #e5e7eb;display:flex;align-items:center;justify-content:center;font-size:10px;color:#9ca3af">Imagen no disponible</div>';
    }).join('')}
      </div>
    </div>`;
  });
...
<head>
  ...
  <title>Inspección Vehicular — ${escapeHtml(data.placa)} — Ballistic Technology</title>
  <style>
...
<div class="page-wrap">
  ...
  <div class="cover">
    <div style="flex:1">
      ...
      <div style="border-top:1px solid rgba(255,255,255,0.15);padding-top:16px">
        ...
        <div class="cover-doc-sub">Documento oficial de evidencia — ${escapeHtml(data.placa)}</div>
        ${data.isLocked
    ...
    <div class="cover-meta">
      <div class="cover-meta-row">
        ...
        <div class="cover-meta-value">${escapeHtml(data.fecha)}</div>
      </div>
      <div class="cover-meta-row">
        <div class="cover-meta-label">Hora</div>
        <div class="cover-meta-value">${escapeHtml(data.hora)}</div>
      </div>
      <div class="cover-meta-row">
        <div class="cover-meta-label">Inspector</div>
        <div class="cover-meta-value">${escapeHtml(data.inspectorName || 'No especificado')}</div>
      </div>
      <div class="cover-meta-row">
        <div class="cover-meta-label">Código</div>
        <div class="cover-meta-value" style="font-family:monospace">${escapeHtml(data.codigoVehiculo || data.id.slice(-8).toUpperCase())}</div>
      </div>
      <div class="cover-meta-row">
        <div class="cover-meta-label">Placa</div>
        <div class="cover-meta-value" style="font-size:18px;color:#93c5fd;letter-spacing:2px">${escapeHtml(data.placa)}</div>
      </div>
      <div class="cover-meta-row">
  ...
  <div class="page-header" id="main-header">
    <div class="page-header-brand">
      ...
      <div>
        ...
        <div class="page-header-sub">Placa: ${escapeHtml(data.placa)} | Inspector: ${escapeHtml(data.inspectorName || '—')} | ID: ${escapeHtml(data.id.slice(-10).toUpperCase())}</div>
      </div>
    </div>
  ...
  <div class="body-content">
    ...
    <div class="section-break">
      <div class="legal-box">
        ...
        <div class="legal-row">
          ...
          <span class="legal-value" style="font-family:monospace;font-size:10px">${escapeHtml(data.id)}</span>
        </div>
        <div class="legal-row">
          <span class="legal-label">Inspector Responsable:</span>
          <span class="legal-value">${escapeHtml(data.inspectorName || '—')}${data.inspectorId ? ` (ID: ${escapeHtml(data.inspectorId.slice(-8).toUpperCase())})` : ''}</span>
        </div>
        <div class="legal-row">
        ...
        <div class="legal-row">
          ...
          <span class="legal-value">${escapeHtml(generatedAt)}</span>
        </div>
    ...
    <div class="section-break">
      ...
      <div class="info-grid">
        <div class="info-label">Marca</div><div class="info-value">${escapeHtml(data.marca || '—')}</div>
        <div class="info-label">Modelo</div><div class="info-value">${escapeHtml(data.modelo || '—')}</div>
        <div class="info-label">Color</div><div class="info-value">${escapeHtml(data.color || '—')}</div>
        <div class="info-label">Placa</div><div class="info-value" style="font-weight:900;color:#1B4F72;font-size:16px;letter-spacing:2px">${escapeHtml(data.placa || '—')}</div>
        <div class="info-label">VIN / Chasis</div><div class="info-value" style="font-family:monospace;font-size:12px">${escapeHtml(data.vin || '—')}</div>
        <div class="info-label">Estado</div><div class="info-value">${escapeHtml(data.estado || '—')}</div>
        <div class="info-label">Propietario</div><div class="info-value">${escapeHtml(data.propietario || '—')}</div>
        <div class="info-label">Teléfono</div><div class="info-value">${escapeHtml(data.telefono || '—')}</div>
        <div class="info-label">Celular</div><div class="info-value">${escapeHtml(data.celular || '—')}</div>
        <div class="info-label">Kilometraje</div><div class="info-value">${escapeHtml(data.km ? data.km + ' km' : '—')}</div>
        <div class="info-label">Bodega</div><div class="info-value">${escapeHtml(data.bodega || '—')}</div>
        <div class="info-label">Nivel</div><div class="info-value">${escapeHtml(data.nivel || '—')}</div>
      </div>
    </div>
    ...
    <div class="section-break">
      ...
      <table class="checklist">
        ...
        <tbody>
          ...
          <tr>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;font-weight:600;color:#374151">${escapeHtml(item.nombre)}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6">
              <span class="badge" style="background:${estadoBg(item.estado)};color:${estadoColor(item.estado)}">${escapeHtml(estadoLabel(item.estado))}</span>
            </td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280;text-align:center">
              ${item.fotos.length > 0 ? `<span style="color:#1B4F72;font-weight:700">${item.fotos.length} foto${item.fotos.length > 1 ? 's' : ''}</span>` : '—'}
            </td>
    ...
    <div class="section-break">
      ...
      <table class="checklist">
        ...
        <tbody>
          ...
          <tr>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;font-weight:600;color:#374151">${escapeHtml(doc.nombre)}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6">
            ...
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280">${escapeHtml(doc.notas || '—')}</td>
          </tr>`).join('')}
        </tbody>
      </table>
    </div>
    ...
    <div class="section-break">
      ...
      <table class="checklist">
        ...
        <tbody>
          ...
          <tr>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;font-weight:600;color:#374151">${escapeHtml(item.nombre)}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280">${escapeHtml(item.categoria)}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6">
              <span class="badge" style="background:${estadoBg(item.estado)};color:${estadoColor(item.estado)}">${escapeHtml(estadoLabel(item.estado))}</span>
            </td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280">${escapeHtml(item.notas || '—')}</td>
    ...
    <div class="section-break">
      ...
      <div style="border:1px solid #e5e7eb;border-radius:10px;padding:16px;display:flex;gap:20px;flex-wrap:wrap">
        <div style="flex:1;min-width:200px">
          ...
          ${data.scanner.notas ? `<div><span style="font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase">Notas</span><p style="margin-top:4px;font-size:12px;color:#374151;line-height:1.6">${escapeHtml(data.scanner.notas)}</p></div>` : ''}
        </div>
        ${scannerImg
    ...
    <div class="section-break">
      ...
      <table class="checklist">
        ...
        <tbody>
          ...
          <tr>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:12px;font-weight:600;color:#374151">${escapeHtml(zona.nombre)}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6">
            ...
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;color:#6b7280">${escapeHtml(zona.notas || '—')}</td>
            <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;font-size:11px;text-align:center">
    ...
    <div class="section-break">
      ...
      <div style="border:1px solid #e5e7eb;border-radius:10px;padding:16px;min-height:80px;background:#fafafa">
        ${data.observaciones
      ? `<p style="font-size:13px;color:#374151;line-height:1.7;white-space:pre-wrap">${escapeHtml(data.observaciones)}</p>`
      ...
      : data.video.tieneVideo === false
          ...
          : `<div>
              ...
              ? `<div style="margin-top:8px;padding:10px;background:#f0f4f8;border-radius:8px;border:1px solid #e5e7eb">
                    ...
                    <a href="${escapeHtml(data.video.videoLink)}" style="color:#1B4F72;font-size:12px;font-weight:600;word-break:break-all">${escapeHtml(data.video.videoLink)}</a>
    ...
    <div class="section-break">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
        <div>
          <p style="font-size:12px;font-weight:700;color:#374151;margin-bottom:8px">Inspector: ${escapeHtml(data.firmas.inspectorName || data.inspectorName || 'No especificado')}</p>
          <div class="sig-box">
            ${inspectorSig
      ...
      : '<p style="font-size:11px;color:#9ca3af;font-style:italic">Sin firma</p>'}
        ...
        <div>
          <p style="font-size:12px;font-weight:700;color:#374151;margin-bottom:8px">Cliente: ${escapeHtml(data.firmas.clienteName || 'No especificado')}</p>
          <div class="sig-box">
            ${clienteSig
  ...
  <div class="page-footer">
    ...
    <div class="page-footer-center">
      Reporte generado: ${escapeHtml(generatedAt)}<br/>
      <span style="font-size:8px">Documento de evidencia legal — No modificar</span>
    </div>
    <div class="page-footer-right">
      ID: ${escapeHtml(data.id.slice(-12).toUpperCase())}<br/>
